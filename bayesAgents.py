...
#    print "varDict = ", variableDomainsDict

#    for var in obsPosList:
#        variableDomainsDict[var] = OBS_VALS
...
def fillObsCPT(bayesNet, gameState):
    ...
    for pos in possibleHouses:
        if pos == bottomLeftPos:
	    ajacentHouseLoc = BOTTOM_LEFT_VAL
        if pos == topLeftPos:
	    ajacentHouseLoc = TOP_LEFT_VAL
        if pos == bottomRightPos:
	    ajacentHouseLoc = BOTTOM_RIGHT_VAL
        if pos == topRightPos:
	    ajacentHouseLoc = TOP_RIGHT_VAL
#	print "POS = ", pos
        obsVars = gameState.getHouseWalls(pos)
#        print "obsVars = ", obsVars
#        print "bayesNet.variableDomainsDict() = ", bayesNet.variableDomainsDict()
        for var in obsVars:
	    ovar = OBS_VAR_TEMPLATE % var
	    obsFactor = bn.Factor([ovar], HOUSE_VARS, bayesNet.variableDomainsDict())
#            for assignment in foodHouseFactor.getAllPossibleAssignmentDicts():
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
#                print assignment
		if ovar in assignment:
		    # Near a FOOD HOUSE
		    if assignment[FOOD_HOUSE_VAR] == ajacentHouseLoc:
		        if assignment[ovar] == RED_OBS_VAL:
			    obsFactor.setProbability(assignment, PROB_FOOD_RED)
		        elif assignment[ovar] == BLUE_OBS_VAL:
			    obsFactor.setProbability(assignment, 1 - PROB_FOOD_RED)
			else: # ovar == none
			    obsFactor.setProbability(assignment, 0)
		    # Near a GHOST HOUSE
		    elif assignment[GHOST_HOUSE_VAR] == ajacentHouseLoc:
		        if assignment[ovar] == RED_OBS_VAL:
            ...
            bayesNet.setCPT(ovar, obsFactor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for i in factor.getAllPossibleAssignmentDicts():
        if curr < factor.getProbability(i):
            curr = factor.getProbability(i)
            loc = i
    #print(loc)
    return loc
    #util.raiseNotDefined()
...
#        print "inference = ", inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder).getAllPossibleAssignmentDicts()
        #print "====================================1"
        for assignment in inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder).getAllPossibleAssignmentDicts():
            #print "ASSIGNMENT: FH=", assignment[FOOD_HOUSE_VAR], "  GH=", assignment[GHOST_HOUSE_VAR]
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                p = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder).getProbability(assignment)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
#        print "getExplorationProbsAndOutcomes(evidence) = ", self.getExplorationProbsAndOutcomes(evidence)
#        print "====================================3"
...
#        print "evidence = ", evidence
        ...
        for assignment in self.getExplorationProbsAndOutcomes(evidence):
            #print "assignment = ", assignment
#            E[value of exploration] = sum p(new evidence) max_{actions} E[action | old evidence and new evidence]
            expectedValue += assignment[0] * max(self.computeEnterValues(assignment[1], enterEliminationOrder))
#        x = self.computeEnterValues(evidence, enterEliminationOrder)
#    def computeEnterValues(self, evidence, eliminationOrder):
#        util.raiseNotDefined()
